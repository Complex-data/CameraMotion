...
for i = 1:1033
    ...
    tic
    [f1p, vp1p] = findAndExtractELASFeatures(I1p, 7, 50);
    display(['Previous image: ELAS features found and extracted in ', ...
                num2str(toc, '%3.2f'), ' s.']);

    tic
    [f1c, vp1c] = findAndExtractELASFeatures(I1c, 7, 50);
    display(['Current image: ELAS features found and extracted in ', ...
                num2str(toc, '%3.2f'), ' s.']); 

    %% Feature matching

    % Try out my own algorithm for matching
    tic
    [ind, met] = findMatches(f1p, f1c, vp1p, vp1c, 30, 1000, true);
    ...
    % Extract coordinates
    uv1p = vp1p(ind2(:, 1), 1:2);
    uv1c = vp1c(ind2(:, 2), 1:2);
...
%     end
    ...
    subplot(4, 1, 4);
    hold off
    showMatchedFeatures(I1p, I1c, ...
        vp1p(ind2(:, 1), 1:2), vp1c(ind2(:, 2), 1:2));
