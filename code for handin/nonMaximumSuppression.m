function ext = nonMaximumSuppression(I1, I2, n, thr)
    ...
    % This algorithm ensures that a local maximum / minimum is only accepted 
    % if there is no larger / smaller value in a (2n + 1) x (2n + 1) window 
    % around it.
    width = size(I1, 2);
    height = size(I1, 1);
    ...
    for i = (n + 1 + margin):(n + 1):(width - n - margin - 1)
        for j = (n + 1 + margin):(n + 1):(height - n - margin - 1)
            % First assume our minima and maxima are at the current position
            I1max_mi = i; I1max_mj = j; I2max_mi = i; I2max_mj = j;
            I1min_mi = i; I1min_mj = j; I2min_mi = i; I2min_mj = j;
            
            % ...and set the current image values as minimum and maximum value
            I1max_val = I1(j, i);
            I1min_val = I1max_val;
            I2max_val = I2(j, i);
            I2min_val = I2max_val;

            % Search for larger / smaller values in a window of n + 1
            % pixels down and to the right of the current position        
            for i2 = i:(i + n)
                for j2 = j:(j + n)
                    cval = I1(j2, i2);
                    if cval < I1min_val
            ...
            % Check if we really found a local maximum / minimum
            % Otherwise discard the position
